...
function preparePath() {
	...
	mkdir -pv -m 777 mysql/conf mysql/data mongo/conf mongo/data postgres/conf postgres/data redis/conf redis/data
	chmod 700 /$ROOT_PATH/.env/tcoffe/db/mysql/conf
	chmod 700 /$ROOT_PATH/.env/tcoffe/db/mongo/conf
	chmod 700 /$ROOT_PATH/.env/tcoffe/db/postgres/conf
	chmod 700 /$ROOT_PATH/.env/tcoffe/db/redis/conf
...
# 数据库(mysql)备份脚本初始化
function initMySqlBackScript() {
	cat /$ROOT_PATH/.env/tcoffe/db/mysql/dbBacker.sh << EOF
#!/bin/sh

distDir=/smpoo_file/backup/db/timing/mysql

mkdir -p -m 777 $distDir
currDate=`date +%Y_%m_%d_%H_%M`
# \$1：mysql密码， \$2：数据库名称
mysqldump -hlocalhost -uroot -p$1 $2 > /root/dbMgr/data/$2-$currDate.sql
EOF
}
echo "*/5 * * * *  /root/dbMgr/backer.sh 49.234.145.11 Smpoo@2015 smpoo" >> /$ROOT_PATH/.env/tcoffe/db/mysql/autoBackup
crontab autoBackup
# 初始化资源包
function initAssets() {
	tipGreen "资源包初始化"
	...
	cd /$ROOT_PATH/project/tcoffe/html/www
