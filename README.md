...
> #### 推荐：(避免防火墙干扰，下载即安装)
```shell
sh -c "$(wget https://cdn.jsdelivr.net/gh/fanzouguo/install-linux@19.2.0/script/install.sh -O -)"
```
> #### 或：
```shell
curl -o- https://cdn.jsdelivr.net/gh/fanzouguo/install-linux@19.2.0/script/install.sh | bash
```

> #### 也可以在初始化时，传入项目名称，项目名称缺省为：tcoffe
```shell
wget https://cdn.jsdelivr.net/gh/fanzouguo/install-linux@19.2.0/script/install.sh | sh [项目名称]
```
